      ...
      container(name: 'postgres') {
        withCredentials([usernamePassword(credentialsId: "${params.cred}", passwordVariable: 'pw',  usernameVariable: 'user')]) {
          sh '''
                ...
                psql -h '''+Dburl+''' -U ${user} -p 5432 -d postgres -f  CREATE_LookUPDB/CREATE_DB_LookupDB.sql
                for entry in CREATE_SCHEMA_LookUPDB/*.sql
                  do
                   if [ -f "$entry" ]; then
                   psql -h '''+Dburl+''' -U ${user} -p 5432 -f $entry -d LookupDB
