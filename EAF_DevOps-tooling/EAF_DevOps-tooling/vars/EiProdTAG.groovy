  ...
  node(POD_LABEL) {
   stage('GetCode') {
    ...
    stage('MavenBuild') {
      ...
      mavenExecution(
        buildParams : "-f EAFIntegratorParentProject/pom.xml install"
        )
      }
    stage('Copy Car') {
          println "Copy Car files"
          fileOperations([fileCopyOperation(excludes: '', flattenFiles: true, includes: 'EAFIntegratorParentProject/*CompositeExporter/target/*.car', renameFiles: false, sourceCaptureExpression: '', targetLocation: 'Config/carbonapps/', targetNameExpression: '')])
          println "Copy Jar files"
          fileOperations([fileCopyOperation(excludes: '', flattenFiles: true, includes: 'EAFIntegratorParentProject/*/target/*.jar', renameFiles: false, sourceCaptureExpression: '', targetLocation: 'Config/lib/', targetNameExpression: '')])    
      }
      def zone = ['NA', 'EU']
