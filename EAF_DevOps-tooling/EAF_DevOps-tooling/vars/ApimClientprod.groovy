...
def call(body) {
  ...
  podTemplate(yaml: AgentTemplate.stripIndent()) {
    node(POD_LABEL) {
      stage('Code checkout') {
        checkout([$class: 'GitSCM', branches: [[name: Branch]], doGenerateSubmoduleConfigurations: false, extensions: [[$class: 'CleanBeforeCheckout', deleteUntrackedNestedRepositories: true], [$class: 'RelativeTargetDirectory', relativeTargetDir: './EAF_Client_APIM_Main'], [$class: 'LocalBranch', localBranch: '**']], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'git-devops', url: 'https://github.com/capgemini-gadm/EAF_Client_APIM_Main.git']]])
                        ...
                        branch: "${GtiBranchPlf}", url: "https://github.com/capgemini-gadm/${GitRepoPlf}", credentialsId: "git-devops")
                         ...
                         def exists = fileExists "platform/${AppName}/${Proj}/${Region}/${Envir}/"
                            ...
                            if (exists) {
                              ...
                              sh "cp -Rpv platform/${AppName}/${Proj}/${Region}/${Envir}/* ./EAF_Client_APIM_Main/"
                              sh "ls -l ./EAF_Client_APIM_Main/"
                              sh "cat ./EAF_Client_APIM_Main/api/CommandTowerAPI/api_params.yaml"
                            }
      ...
      stage('config apictl') {
        ...
        stage('Config setup') {
          ...
          //fileOperations([folderCopyOperation(destinationFolderPath: './EAF_Client_APIM_Main', sourceFolderPath: "devops/helm-chart/config/${AppEnv}")])
          //sh "cp -Rpv devops/helm-chart/config/${AppEnv}/ ./EAF_Client_APIM_Main/"
          fileOperations([folderCopyOperation(destinationFolderPath: 'EAF_Client_APIM_Main', sourceFolderPath: "EAF_Client_APIM_Main/devops/helm-chart/config/${AppEnv}/")])
          println 'config setup done...'
        }
        //sh "pwd;sleep 500"
        stage('Deploy APIs') {
          def apim_config = readYaml file: './EAF_Client_APIM_Main/deployment.yaml'
          ...
          if (apisToDeploy != null) {
            ...
            apisToDeploy.each { Module, apisList->
              ...
              def apis = apisList
              for (String api:apis) {
                //import_api = "apictl import-api -f "+"./EAF_Client_APIM_Main/api/"+"${api}"+" -e "+"${Env}"+" -k --preserve-provider=false --update"
                import_api = "apictl import api -f "+"./EAF_Client_APIM_Main/api/"+"${api}"+" -e "+"${Env}"+" --params "+"./EAF_Client_APIM_Main/api/" + "${api}"+"/api_params.yaml"+" --preserve-provider=false --rotate-revision --update -k"
                println '${import_api}'
                sh import_api
        ...
        stage('Deploy Apps') {
          ...
          if (appsToDeploy != null) {
            ...
            for (String app:appsToDeploy) {
              ...
              //import_app = "apictl import-app -f "+"./EAF_Client_APIM_Main/apps/"+"${app}.zip"+" -s=true"+" -e "+"${Env}"+" -k --update"
              import_app = 'apictl import app -f ' + './EAF_Client_APIM_Main/apps/' + "${app}.zip" + ' -e ' + "${Env}" + ' -s=false --preserve-owner=true -k --update'
              //import_app = 'apictl import app -f ' + './EAF_Client_APIM_Main/apps/' + "${app}.zip" + ' -e ' + "${Env}" + ' -s=false -k --update'
              sh import_app
