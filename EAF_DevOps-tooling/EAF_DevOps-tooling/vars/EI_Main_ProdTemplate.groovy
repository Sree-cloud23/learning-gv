 ...
 node(POD_LABEL) {
   ...
   stage('GetCode') {
    ...
    // START: Copy environment config files
      sh "pwd && ls -lrht"
      stage('ConfigUpdate') {
        def exists = fileExists "devops/helm-chart/config/${HelmEnv}"
        echo "===${exists}==="
        if (exists) {
          //sh "pwd"
    ...
    stage('MavenBuild') {
      ...
      mavenExecution(
        buildParams : "-f CIAPIntegratorParentProject/pom.xml install"
        )
      }
    stage('Copy Car') {
          println "Copy Car files"
          fileOperations([fileCopyOperation(excludes: '', flattenFiles: true, includes: 'CIAPIntegratorParentProject/*CompositeApplication/target/*.car', renameFiles: false, sourceCaptureExpression: '', targetLocation: 'Config/carbonapps/', targetNameExpression: '')])
          println "Copy Jar files"
          fileOperations([fileCopyOperation(excludes: '', flattenFiles: true, includes: 'CIAPIntegratorParentProject/*/target/*.jar', renameFiles: false, sourceCaptureExpression: '', targetLocation: 'Config/lib/', targetNameExpression: '')])    
      }
     ...
     // Helm Release starts
      ...
      dir("${ReleaseDir}") {
        ...
        try {
          checkout([$class: 'GitSCM', branches: [[name: HelmBranch]], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'up-bitbucket', url: 'https://github.com/capgemini-gadm/eaf-helm-chart.git']]])
        } catch(Exception err) {
   ...
   // Helm Release ends
      stage('Helm Chart Verify') {
       verifyHelmChartEI(HelmEnv, ReleaseDir, HelmCharName, gitCommit2, NameSpace)
      }
   
   // Helm Deployemnt 
    stage('Helm Chart Deployment') {  
    deployHelmChartEI(HelmEnv, HelmName, ReleaseDir, HelmCharName, gitCommit2, NameSpace, BUILD_NUMBER)
    }
 }
}
    }
