 ...
 node(POD_LABEL) {
   ...
   stage('GetCode') {
    ...
    stage('MavenBuild') {
      ...
      mavenExecution(
        buildParams : "-f EAFTaskIntegratorParentProject/pom.xml install"
        )
      }
    stage('Copy Car') {
          println "Copy Car files"
          fileOperations([fileCopyOperation(excludes: '', flattenFiles: true, includes: 'EAFTaskIntegratorParentProject/*CompositeExporter/target/*.car', renameFiles: false, sourceCaptureExpression: '', targetLocation: 'Config/carbonapps/', targetNameExpression: '')])
          println "Copy Jar files"
          fileOperations([fileCopyOperation(excludes: '', flattenFiles: true, includes: 'EAFTaskIntegratorParentProject/*/target/*.jar', renameFiles: false, sourceCaptureExpression: '', targetLocation: 'Config/lib/', targetNameExpression: '')])    
      }
