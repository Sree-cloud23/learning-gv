...
container('utils') {
        ...
        printTime("Sucessfully entired into ReleaseDir for release : ${ReleaseDir}/${HelmCharName}")
          ...
          retry(2) {
            ...
            sh "helm upgrade --install ${HelmName} devops/helm-chart/${HelmCharName}  --namespace ${NameSpace} --values devops/helm-chart/config/${AppEnv}/values.yaml --values devops/helm-chart/config/${AppEnv}/configmap.yaml --set-string image.tag=${gitCommit2} --set-string image.Envir=${Envir} --set-string image.Region=${Region} --description jenkins-build-${BUILD_NUMBER}"
            //}
           }
          }
         }
