        ...
        dir("${ReleaseDir}/${HelmCharName}") {
          ...
          try {
            //withKubeConfig(credentialsId: clusterCredentials, serverUrl: clusterAddress) {
              def helm = sh(script: "helm lint devops/helm-chart/${HelmCharName} --values devops/helm-chart/config/${AppEnv}/values.yaml --values devops/helm-chart/config/${AppEnv}/configmap.yaml", returnStdout: true).trim() 
            //}
          } catch(Exception err) {
