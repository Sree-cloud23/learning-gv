        ...
        withCredentials([usernamePassword(credentialsId: 'git-devops', passwordVariable: 'password', usernameVariable: 'userName')]) {
          ...
          sh '''
            sh_command_id=$(aws ssm send-command \
                ...
                --parameters '{"commands":["cd /tmp && git clone -b VM_ml https://'''+gituser+''':'''+gitpass+'''@github.com/capgemini-gadm/EAF_DevOps.git && cd /tmp/EAF_DevOps/ && sudo sh -x Ikon_ml_Prod.sh '''+GitBranch+''' '''+Region+''' '''+Envir+''' '''+gituser+''' '''+gitpass+''' && rm -fr /tmp/EAF_DevOps/"]}' \
                --output text \
