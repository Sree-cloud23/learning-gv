      ...
      container(name: 'postgres') {
        withCredentials([usernamePassword(credentialsId: "${params.cred}", passwordVariable: 'pw',  usernameVariable: 'user')]) {
          sh '''
                ...
                psql -h '''+Dburl+''' -U ${user} -p 5432 -d postgres -f  CREATE_CommonConfigDB/CREATE_DB_CommonConfigDB.sql
                for entry in CREATE_SCHEMA_CommonConfigDB/*.sql
                  do
                   if [ -f "$entry" ]; then
                   psql -h '''+Dburl+''' -U ${user} -p 5432 -f $entry -d CommonConfigDB
